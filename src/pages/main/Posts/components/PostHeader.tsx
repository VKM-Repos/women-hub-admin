...
const PostHeader = ({ step, post, handleGoBack, handleSaveToDraft, handlePublish, handleUpdate }: Props) => {
    ...
    return (
        <header className="grid grid-cols-2 items-center justify-between min-h-[5rem] p-4">
            <div className="w-full col-span-1 flex items-center justify-start gap-4">
                ...
                <h2 className="text-xl font-semibold w-full max-w-[300px] truncate">{post?.title ? post.title : 'Add post'}</h2>
            </div>
            {step > 1 && (
